...
export async function showStartDialog(settings: BridgeSettings, currentProject: string) {
  ...
  const form = createEl("form"), status = createEl("div", { className: "obsidian-chatgpt-bridge-modal-status" });
  ...
  const row = createEl("div", { className: "obsidian-chatgpt-bridge-inline-actions" }); row.append(createInput, createButton);
  ...
  const actions = createEl("div", { className: "obsidian-chatgpt-bridge-modal-actions" }); const cancel = createEl("button", { text: "Cancel", attrs: { type: "button" } }); const start = createEl("button", { text: "Start", attrs: { type: "submit" } }); start.className = "obsidian-chatgpt-bridge-primary"; actions.append(cancel, start);
  form.append(createEl("label", { text: "Default project for this session", className: "obsidian-chatgpt-bridge-field-label" }), select, createSection, status, actions);
  panel.append(createEl("h2", { text: "Start Obsidian Bridge" }), createEl("p", { className: "obsidian-chatgpt-bridge-modal-copy", text: "Click Start to continue immediately, or optionally set a default project for this session." }), form);
