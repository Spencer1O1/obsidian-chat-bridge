...
export async function showFileExplorerDialog(options: ExplorerOptions) {
  ...
  const done = createEl("button", { text: options.doneLabel || "Done", attrs: { type: "button" } }); done.className = "obsidian-chatgpt-bridge-primary";
  actions.append(cancel, done); toolbar.append(back, path); listWrap.appendChild(list);
  selectedSection.append(createEl("div", { className: "obsidian-chatgpt-bridge-file-heading", text: "Selected Files" }), selectedWrap);
  body.append(toolbar, status, listWrap);
  if (options.showSelectionSummary) body.append(selectedSection);
  panel.append(createEl("h2", { text: options.title }), createEl("p", { className: "obsidian-chatgpt-bridge-modal-copy", text: options.subtitle }), body, actions);
  ...
  const renderSelected = () => { if (!options.showSelectionSummary) return; selectedWrap.innerHTML = ""; const paths = sortNames([...state.selectedPaths]); if (!paths.length) return selectedWrap.appendChild(createEl("div", { className: "obsidian-chatgpt-bridge-empty-state", text: "No files selected yet." })); paths.forEach(item => { const row = createEl("div", { className: "obsidian-chatgpt-bridge-selected-item" }); const remove = createEl("button", { text: "Remove", className: "obsidian-chatgpt-bridge-secondary-action", attrs: { type: "button" } }); remove.addEventListener("click", () => { state.selectedPaths.delete(item); renderSelected(); renderItems(state.lastItems); }); row.append(createEl("div", { className: "obsidian-chatgpt-bridge-selected-path", text: item }), remove); selectedWrap.appendChild(row); }); };
  const renderItems = (items: typeof state.lastItems) => { state.lastItems = items; list.innerHTML = ""; path.textContent = state.dir ? `${state.dir}/` : "/"; back.textContent = state.dir ? "< Back" : options.rootLabel; back.disabled = !state.dir; if (!items.length) return list.appendChild(createEl("div", { className: "obsidian-chatgpt-bridge-empty-state", text: "This folder is empty." })); items.forEach(item => { if (item.isDirectory) { const button = createEl("button", { className: "obsidian-chatgpt-bridge-browser-entry", attrs: { type: "button" } }); button.addEventListener("click", () => browse(item.path)); button.append(createEl("span", { className: "obsidian-chatgpt-bridge-browser-name", text: item.name }), createEl("span", { className: "obsidian-chatgpt-bridge-browser-kind", text: "Folder >" })); const row = createEl("div", { className: "obsidian-chatgpt-bridge-browser-row" }); row.appendChild(button); return list.appendChild(row); } if (!/\.md$/i.test(item.name)) return; const row = createEl("label", { className: "obsidian-chatgpt-bridge-browser-file" }); const input = createEl("input", { attrs: { type: "checkbox", value: item.path } }); input.className = "obsidian-chatgpt-bridge-checkbox-input"; input.checked = state.selectedPaths.has(item.path); input.addEventListener("change", () => { input.checked ? state.selectedPaths.add(item.path) : state.selectedPaths.delete(item.path); renderSelected(); }); row.append(input, createEl("span", { text: item.name, className: "obsidian-chatgpt-bridge-checkbox-label" })); list.appendChild(row); }); if (!list.children.length) list.appendChild(createEl("div", { className: "obsidian-chatgpt-bridge-empty-state", text: "No markdown files or folders were found here." })); };
